...
export function createModel(json) {
	...
	loader.initMaterials(geometry, json.materials, '');
	...
	function parseModel(scale) {
		...
		while (offset < zLength) {
			vertex = new THREE.Vector3(vertices[offset++] * scale, vertices[offset++] * scale, vertices[offset++] * scale);
	...
	function parseMorphing(scale) {
		...
		if (json.morphTargets !== undefined) {
			...
			for (i = 0, l = json.morphTargets.length; i < l; i++) {
				...
				for (v = 0, vl = srcVertices.length; v < vl; v += 3) {
					...
					dstVertices.push(new THREE.Vector3(new THREE.Vector3(x, y, z)));
		...
		if (json.morphColors !== undefined) {
			...
			for (i = 0, l = json.morphColors.length; i < l; i++) {
				...
				geometry.morphNormals [i].name = json.morphColors[i].name;
				geometry.morphNormals [i].colors = [];

				dstColors = geometry.morphNormals [i].colors;
